...
#VOLUMELIBRARY2 does not have the constant variable I3, I7, I15, I21
print('Test VOLUMELIBRARY2 for biomass output')
mydll.VOLUMELIBRARY2(byref(regn), forst.str,forst.len,voleq.str,voleq.len, byref(MTOPP), byref(MTOPS), byref(STUMP),byref(DBHOB),
byref(DRCOB), HTTYPE.str,HTTYPE.len, byref(HTTOT), byref(HTLOG), byref(HT1PRD), byref(HT2PRD), byref(UPSHT1), byref(UPSHT2), byref(UPSD1),
byref(UPSD2), byref(HTREF), byref(AVGZ1), byref(AVGZ2), byref(FCLASS), byref(DBTBH), byref(BTR), 
byref(VOL), byref(LOGVOL), byref(LOGDIA), byref(LOGLEN), byref(BOLHT), byref(TLOGS), byref(NOLOGP), byref(NOLOGS), byref(CUTFLG),
byref(BFPFLG), byref(CUPFLG), byref(CDPFLG), byref(SPFLG), CONSPEC.str,CONSPEC.len,PROD.str, PROD.len, byref(HTTFLL),LIVE.str,LIVE.len,
byref(BA) , byref(SI),mCTYPE.str,mCTYPE.len, byref(ERRFLAG), byref(idist), byref(HTBRK), byref(HTBRKD), byref(FIASPCD), byref(DRYBIO), byref(GRNBIO),
 byref(CR), byref(CULL), byref(DECAYCD))

print ('ERRFLAG = ', ERRFLAG.value)
for index, item in enumerate(GRNBIO):
	print ('GRNBIO[',index+1,'] = ',item)

for index, item in enumerate(DRYBIO):
	print ('DRYBIO[',index+1,'] = ',item)
